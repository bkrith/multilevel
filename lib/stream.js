    ...
    constructor(db, events, opts) {
        ...
        this._write = function (chunk, enc, cb) {
            ...
            this.getChunk().forEach((binPacket) => {
                const packet = jsBinary_1.jsBinary.packet.decode(binPacket);
                if (db && packet.data.parameters) {
                    if (packet.data.method.includes('Stream')) {
                        db[packet.data.method](...packet.data.parameters)
                            .on('data', (data) => {
                            this.push(jsBinary_1.jsBinary.packet.encode(this.formatPacket(packet, data, undefined)));
                        })
                            .on('end', () => {
                            this.push(jsBinary_1.jsBinary.packet.encode(this.formatPacket(packet, undefined, undefined, true)));
                        })
                            .on('error', (err) => {
                            this.push(jsBinary_1.jsBinary.packet.encode(this.formatPacket(packet, undefined, err)));
                        });
                    }
                    else {
                        db[packet.data.method](...packet.data.parameters, (err, value) => {
                            this.push(jsBinary_1.jsBinary.packet.encode(this.formatPacket(packet, value, err)));
                        });
                    }
                }
                else {
                            ...
                            switch (packet.data.method) {
                                case 'createReadStream':
                                    if (packet.data.result)
                                        events.readStream.emit('data', packet.data.result);
                                    else if (packet.data.end)
                                        events.readStream.emit('end');
                                    else if (packet.data.error)
                                        events.readStream.emit('error', packet.data.error);
                                    break;
                                case 'createKeyStream':
                                    if (packet.data.result)
                                        events.keyStream.emit('data', packet.data.result);
                                    else if (packet.data.end)
                                        events.keyStream.emit('end');
                                    else if (packet.data.error)
                                        events.keyStream.emit('error', packet.data.error);
                                    break;
                                case 'createValueStream':
                                    if (packet.data.result)
                                        events.valueStream.emit('data', packet.data.result);
                                    else if (packet.data.end)
                                        events.valueStream.emit('end');
                                    else if (packet.data.error)
                                        events.valueStream.emit('error', packet.data.error);
                                    break;
                            }
                        }
                        else {
