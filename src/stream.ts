    ...
    constructor(db?: any, events?: MultiLevel, opts?: any) {
        ...
        this._write = function (chunk: Buffer, enc: any, cb: any) {
            ...
            this.getChunk().forEach((binPacket: Buffer) => {
                const packet = jsBinary.packet.decode(binPacket);

                if (db && packet.data.parameters) {
                    if (packet.data.method.includes('Stream')) {
                        db[packet.data.method](...packet.data.parameters)
                            .on('data', (data: any) => {
                                this.push(jsBinary.packet.encode(this.formatPacket(packet, data, undefined)));
                            })
                            .on('end', () => {
                                this.push(jsBinary.packet.encode(this.formatPacket(packet, undefined, undefined, true)));
                            })
                            .on('error', (err: any) => {
                                this.push(jsBinary.packet.encode(this.formatPacket(packet, undefined, err)));
                            });
                    }
                    else {
                        db[packet.data.method](...packet.data.parameters, (err: any, value?: any) => {
                            this.push(jsBinary.packet.encode(this.formatPacket(packet, value, err)));
                        });
                    }
                }
                else {
